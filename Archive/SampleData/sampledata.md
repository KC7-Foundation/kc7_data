...
![d6cdb9dd-3857-4328-b2f6-b67e7b1e911b](https://github.com/KC7-Foundation/internal-actor-configs/assets/9474932/fa974390-4ae3-4a65-b1ff-3b34c1e2d98f)
...
ADX is the primary tool used in the Spider Society SOC for data exploration and analysis. The great thing about ADX is that it is used by cyber analysts at many of the smallest and largest organizations in the world. 

Let’s get you logged in and started with ADX:

1. Go to the Spider Society training module at [kc7cyber.com/SpiderSociety](http://kc7cyber.com/SpiderSociety)
...
<img width="462" alt="image" src="https://github.com/KC7-Foundation/internal-actor-configs/assets/9474932/1d37b5e1-5809-4c9d-8012-08c26d2e2568">

Data in ADX is organized in a hierarchical structure which consists of **clusters**, **databases**, and **tables**.

<img width="473" alt="image" src="https://github.com/KC7-Foundation/internal-actor-configs/assets/9474932/1b34a696-5d95-4852-8ebc-4665d7f9a9f9">

All of Spider Society’s security logs are stored in a single database – the SpiderVerse database. 

  2. Select your database.  
	- Expand the dropdown arrow next to the SpiderVerse database.
	- Click on the **Spider Society** database. Once you’ve done this, you should see the database highlighted- this means you’ve selected the database and are ready to query the tables inside.

Note: It’s very important that you use the SpiderVerse database for all questions while you’re investigating activity at Spider Society! If you choose the wrong database, you won’t be able to answer questions correctly.
...
Currently, you’ll see there’s a message there welcoming you to Spider Society! Click the blue Run button above the query workspace to run your first query! Once you’ve done that, you can erase the welcome message by highlighting it and pressing backspace or delete on your keyboard.

Okay, enough introductions… let’s get your hands on the data.

#### First Look at the data... 

The **Spiderman Organization** database contains nine tables. Tables contain many rows of similar data. For security logs, a single row typically represents a single thing done by an employee or a device on the network at a particular time.

We currently have nine types of log data. As you’ll see in ADX, each log type corresponds to a table that exists in the **Spiderman Organization** database:
...
The Employees table contains information about all the employees in our organization. In this case, we can see that the organization is named “Spider Society” and the domain is “spiderverse.org”.
...
We can use the **where** operator in KQL to apply filters to a particular field. For example, we can find all the employees with the name “James” by filtering on the name column in the Employees table.
...
```sql
Employees
| where name == "James Morano"
```

>3. 🤔Each employee at Spider Society is assigned an IP address. Which employee has the IP address: “192.168.2.191”?
...
While performing their day-to-day tasks, Spider Society employees send and receive emails. A record of each of these emails is stored in the **Email** table.
...
We can find information about the emails sent or received by a user by looking for their email address in the sender and recipient fields of the **Email** table. For example, we can use the following query to see all the emails sent by “James Morano”:

```sql

Email
| where sender == "james_morano@spiderverse.org"

```

> 4. 🤔How many emails did Gregg Smith receive?
...
```sql
Email
| where sender has "spiderverse.org"
| distinct sender
| count
```
...
In line 2, we take the Email table and filter the data down to find only those rows with “spiderverse.org” in the sender column.

In line 3, we add another pipe character ( | ) and use the distinct operator to find all the unique senders. Here, we aren’t finding the unique senders for all of the email senders, but only the unique senders that are left after we apply the filter looking for rows with “spiderverse.org” in the sender column.
...
You’ve finished your training and you’re ready to get to work protecting Spider Society. 
