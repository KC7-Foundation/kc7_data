...
We can use the **where** operator in KQL to apply filters to a particular field. For example, we can find all the employees with the name “Zara” by filtering on the name column in the Employees table.
...
We can find information about the emails sent or received by a user by looking for their email address in the sender and recipient fields of the **Email** table. For example, we can use the following query to see all the emails sent by “Zara Choudhury”:

```sql

Email
| where sender == "zara_choudhury@deathcon-scholomance.io"

```

> 4. 🤔How many emails did Elsie Hall receive?
...
```sql
Email
| where sender has "deathcon-scholomance.io"
| distinct sender
| count
```
...
In line 2, we take the Email table and filter the data down to find only those rows with “deathcon-scholomance.io” in the sender column.

In line 3, we add another pipe character ( | ) and use the distinct operator to find all the unique senders. Here, we aren’t finding the unique senders for all of the email senders, but only the unique senders that are left after we apply the filter looking for rows with “deathcon-scholomance.io” in the sender column.
...
5.  🤔How many distinct senders were seen in the email logs from scholomance-suppliers.online?
...
The query above tells us their IP address is “10.10.0.97”. We can take their IP address and look in the **OutboundNetworkEvents** table to determine what websites they visited.

```sql
OutboundNetworkEvents
| where src_ip == "10.10.0.97"
```

> 6.  🤔How many unique websites did “Becky Thrower” visit?
...
> 7.  🤔 How many domains in the PassiveDns records contain the word “documents”? (hint: use the contains operator instead of has. If you get stuck, do a take 10 on the table to see what fields are available.)
> 8. 🤔 What IPs did the domain “information-documents.org” resolve to (enter any one of them)?
